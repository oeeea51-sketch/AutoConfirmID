...
public:
    bool init(EffectGameObject* trigger, cocos2d::CCArray* objects) {
        ...
        // Создаем зеленую кнопку "Confirm"
        ...
        // Ставим кнопку по центру внизу меню
        confirmBtn->setPosition({0.f, -115.f});

        // Добавляем кнопку в интерфейс триггера
    ...
    void onConfirmID(cocos2d::CCObject*) {
        ...
        // Ищем предыдущий объект, у которого есть группа
        ...
        // Получаем ID первой группы объекта
        int groupID = targetObj->getGroupID(0);
        if (groupID <= 0) return;

        // Вставляем ID в текстовое поле Move-триггера
        if (this->m_targetGroupInput) {
            this->m_targetGroupInput->setString(std::to_string(groupID).c_str());
        }

        // Записываем ID в сам триггер для сохранения в игре
