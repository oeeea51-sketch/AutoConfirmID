...
public:
    bool init(EffectGameObject* trigger, cocos2d::CCArray* objects) {
        ...
        // Создаем красивую зеленую кнопку "Confirm"
        ...
        // Позиция кнопки снизу по центру меню
        confirmBtn->setPosition({0.f, -115.f});

        // Безопасно добавляем кнопку в меню триггера
    ...
    void onConfirmID(cocos2d::CCObject*) {
        ...
        // Ищем объект, который был добавлен или изменен прямо перед триггером
        ...
        if (!targetObj) {
            FLAlertLayer::create("Error", "No object with Group ID found!", "OK")->show();
            return;
        }

        // Берем самую первую группу объекта
        int groupID = targetObj->m_groups->at(0);
        if (groupID <= 0) return;

        // Записываем ID в текстовое поле Move-триггера
        if (this->m_targetGroupInput) {
            this->m_targetGroupInput->setString(std::to_string(groupID).c_str());
        }

        // Обновляем значение в самом триггере, чтобы игра его запомнила
